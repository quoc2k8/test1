...
def send_email(text):
    ...
    if 'nghĩa' in recipient: # 'nghĩa' ở đây là keywords để máy tiếp tục gửi email cho bạn. Bạn có thể thay cái keywords này
        ...
        mail.ehlo()
        mail.starttls()
        mail.login('xyz', 'abc') # 'xyz' ở đây là địa chỉ email của bạn (địa chỉ email gửi), 'abc' là mật khẩu của email đó
        mail.sendmail('xyz', 
                      '123', content.encode('utf-8')) # 'xyz' ở đây cũng như bên trên, nhưng '123' là địa chỉ email nhận (email được bạn gửi thư)
        mail.close()
...
def introduce():
    speak("Xin chào bạn. Rất hân hạnh được phục vụ bạn. Tôi là Alex. Tôi là trợ lý ảo được tạo ra dựa trên ngôn ngữ lập trình Python kết hợp với AI. Tôi sinh ra vào ngày 4/10/2020 và được sáng lập bởi Lê Phước Nghĩa. Hiện tại bạn đang sử dụng phiên bản Alex 2.1 (cập nhật lần gần nhất ngày 12/10/2020) và cũng đang là phiên bản mới nhất.")
